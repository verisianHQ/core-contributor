...
class TestRunner:
    ...
    def __init__(
        ...
        latest_codelist_path = max(
            glob(f"engine/{cache_dir}/{self.standard}ct-[0-9][0-9][0-9][0-9]-*.pkl"), default=None
        )
                    ...
                    if case_dir.is_dir():
                        ...
                        if has_csv:
                            cases[test_type].append(
                                {"case_id": case_dir.name, "data_path": str(data_dir), "format": "csv"}
                            )
                        elif has_xlsx:
                            cases[test_type].append(
                                {"case_id": case_dir.name, "data_path": str(data_dir), "format": "xlsx"}
                            )
    ...
    def _read_library_specs_xlsx(excel_path: str) -> Tuple[str, str, List[str]]:
        ...
        try:
            ...
            version = str(first_row[1]).strip().replace("-", ".")
            ct_list = [
                f"{str(row_data[0]).strip()}-{str(row_data[1]).strip()}"
                for row_data in row_iter
                if row_data[0] and str(row_data[0]).strip().lower().endswith("ct") and row_data[1]
            ]
            return standard, version, ct_list
        finally:
            wb.close()
            ...
            if dataset_path.exists():
                ...
                for i, col in enumerate(dataset_df.columns):
                    ...
                    variables.append(
                        VariableMetadata(
                            name=col,
                            label=str(dataset_df[col].iloc[0]),
                            type=var_type,
                            length=dataset_df[col].iloc[2],
                            format="",
                            order=i + 1,
                        )
                    )
    ...
    def run_validation(self, rule_id: str, case_info: dict) -> Tuple[Any, Optional[dict]]:
        ...
        for file in ["define.xml", "stf.xml"]:
            file_path = (
                str(data_path_obj / file)
                if (data_path_obj / file).exists()
                else str(rule_path / file) if (rule_path / file).exists() else None
            )
            if file == "define.xml":
                ...
                if define_xml_path:
                    ...
                    extensible_terms = DefineXMLReaderFactory.from_filename(
                        define_xml_path
                    ).get_extensible_codelist_mappings()
    ...
    def evaluate_case(self, rule_id: str, test_type: str, case_info: dict) -> dict:
        ...
        if results_data is None or results_data.get("error"):
            results_path = ResultReporter.save_case_results(
                ...
                test_type,
                case_id,
                results_data or {"error": "Unknown", "exception": "Engine returned None"},
                self.version_info,
            )
            return {
                "case_id": case_id,
        ...
        if test_type == "negative":
            validations = (
                self.get_validation_info_csv(case_info["data_path"])
                if is_csv
                else self.get_validation_info_xlsx(case_info["data_path"])
            )
        ...
        for sheet in wb.worksheets:
            for row in sheet.iter_rows():
                for cell in row:
                    ...
                    if not fg:
                        continue
                    is_yellow = (isinstance(fg.rgb, str) and fg.rgb.lower().endswith("ffff00")) or (
                        isinstance(fg.indexed, int) and fg.indexed in YELLOW_INDICES
                    )
                    if is_yellow:
                        highlighted_cells.setdefault(sheet.title, {}).setdefault(int(cell.row), {}).update(
                            {sheet.cell(row=1, column=cell.column).value: cell.value}
                        )
                ...
                if not error_obj.get("error"):
                    ...
                    for (v_sheet, v_error_level, v_row, v_idx), v_values in flat_validation.items():
                        if v_sheet.split(".")[0].lower() == ds["dataset"].split(".")[0].lower():
                            ...
                            res_not_absent = set(k for k, v in error_obj["value"].items() if v != "[ABSENT]") - set(
                                k for k, v in v_values.items() if v == "[ABSENT]"
                            )
...
def main():
    ...
    if not args.all_rules:
        ...
        if bundle_path.is_dir() and (bundle_path / "shared_test_cases").exists():
            rules_to_run = sorted(
                [d.name for d in bundle_path.iterdir() if d.is_dir() and d.name != "shared_test_cases"]
            )
            ...
            specific_case = (
                ...
                else InteractiveHandler.prompt_case(runner.get_test_cases(rule_input)) if not args.rule else None
            )
        else:
    ...
    for i, rule_id in enumerate(rules_to_run, 1):
        ...
        if any(t.get("error") for t in summary["positive_tests"] + summary["negative_tests"]):
...
def generate_rule_results(standard: str, rules_dir: str, rule_id: str) -> dict:
    ...
    def dict_string_check(s: str, content: str) -> bool:
        if f"operator: is_valid_{s}" in content or f"operator: is_not_valid_{s}" in content:
            return True
        elif f"external_dictionary_type: {s}" in content:
            return True
        else:
            return False
    ...
    return TestRunner(
        standard=standard,
        unii_path="dummy_ex_dicts/unii" if dict_string_check("unii", content) else None,
        medrt_path="dummy_ex_dicts/medrt" if dict_string_check("medrt", content) else None,
        loinc_path="dummy_ex_dicts/loinc" if dict_string_check("loinc", content) else None,
        snomed_path="dummy_ex_dicts/snomed" if dict_string_check("snomed", content) else None,
