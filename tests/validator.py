...
def normalize_dataset(dataset: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "errors" in normalized:
        normalized["errors"] = sorted(normalized["errors"], key=lambda e: (e.get("row", 0), str(sorted(e.items()))))

    return normalized


def compare_errors(
    committed_errors: List[Dict], generated_errors: List[Dict], test_type: str, case_id: str
) -> Tuple[bool, List[str]]:
    """
    ...
    if len(committed_errors) != len(generated_errors):
        differences.append(
            f"Error count mismatch: committed={len(committed_errors)}, generated={len(generated_errors)}"
        )
